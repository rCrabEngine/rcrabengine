// Rotating cube example using wgpu
// Demonstrates: vertex rotation, per-frame uniform updates
...
impl RotatingCube {
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        let texture_view = output.texture.create_view(&Default::default());
        ...
        let rotation = elapsed * 0.5; // Rotate at 0.5 radians per second

        // Create rotated vertices on CPU
        let base_vertices = create_cube();
        let (sin_r, cos_r) = rotation.sin_cos();

        let rotated_vertices: Vec<Vertex> = base_vertices.iter().map(|v| {
            let x = v.position[0] * cos_r - v.position[2] * sin_r;
            let z = v.position[0] * sin_r + v.position[2] * cos_r;
            Vertex {
                position: [x, v.position[1], z],
                color: v.color,
            }
        }).collect();

        // Upload rotated vertices
        self.queue.write_buffer(&self.vertex_buffer, 0, bytemuck::cast_slice(&rotated_vertices));
...
fn main() {
    ...
    info!("Starting rCrabEngine Rotating Cube Example");
    ...
    let window = event_loop
        .create_window(winit::window::WindowAttributes::default()
            .with_title("rCrabEngine - Rotating Cube")
            .with_inner_size(winit::dpi::LogicalSize::new(800, 600)))
        .unwrap();

    let window_id = window.id();
    let mut app = pollster::block_on(RotatingCube::new(window));
